...
const PERSIST_JITTER_STRENGTH = 0.002; // Increased for smokier jitter
...
const SmokeCanvas: React.FC<SmokeCanvasProps> = ({
  ...
  const initParticles = useCallback((
    ...
    for (let i = 0; i < safeCount; i++) {
      ...
      if (currentParticleSourceType === "Text" && textPointsRef.current.length > 0) {
    ...
    material.onBeforeCompile = shader => {
        ...
        shader.vertexShader = `
            ...
            // uniform float scale; // scale is already provided by PointsMaterial
            ${shader.vertexShader}
        `;
        
        // Ensure we replace the correct line if it exists, or inject if it doesn't.
        // The default PointsMaterial vertex shader contains 'gl_PointSize = size;'
        ...
        if (shader.vertexShader.match(defaultPointSizeAssignmentRegex)) {
            shader.vertexShader = shader.vertexShader.replace(
                ...
                `gl_PointSize = particleSize * 1.3;` // Use our custom attribute
            );
        } else {
            // Fallback: If the exact line isn't found (e.g. due to three.js version changes),
            // try to inject it before size attenuation logic.
            // This is less robust. The regex match is preferred.
            shader.vertexShader = shader.vertexShader.replace(
        ...
        shader.vertexShader = shader.vertexShader.replace(
            ...
            `#include <logdepthbuf_vertex>
             gl_PointSize = max(1.0, gl_PointSize); // Clamp after potential attenuation
             vAlpha = alpha;
             vRotation = 0.0; // Example
            `
        );
        ...
        // Updated map_particle_fragment replacement
        shader.fragmentShader = shader.fragmentShader.replace(
 ...
 const generateTextPoints = useCallback((text: string) => {
    ...
    for (let yPos = 0; yPos < canvas.height; yPos += step) {
        for (let xPos = 0; xPos < canvas.width; xPos += step) {
            if (data[(yPos * canvas.width + xPos) * 4 + 3] > 128 && Math.random() < TEXT_SAMPLE_DENSITY * step * step) {
                const sceneX = (xPos - canvas.width / 2) * TEXT_SHAPE_SCALE;
                const sceneY = -(yPos - canvas.height / 2) * TEXT_SHAPE_SCALE;
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     ...
     const animate = () => {
       ...
       if (safeDelta <= 0 && isPlaying) return;
       time += safeDelta;
       ...
       if (isPlaying) {
          ...
          const isTextShapingMode = !!particleText && textPointsRef.current.length > 0;
          const isCurrentlyPersistingText = isTextShapingMode && persistTextShape;
          ...
          const updateParticles = (
              ...
              if (!attrs.position || !attrs.velocity || !attrs.alpha || !attrs.particleSize || !attrs.life || !attrs.color || !attrs.turbulenceOffset || !attrs.targetTextPoint || !attrs.randomFactors) {
                   console.warn("Particle system attributes missing, skipping update for:", isFire ? "fire" : "smoke");
                   return;
              }
              ...
              if (isTextShapingMode) {
                // General text shaping defaults
                effSpread = isFire ? 0.3 : 0.25;
                effTurbulence = 0.1;
                effSpeed = 0.001;
                effOpacity = isFire ? 0.9 : 0.85;
                if (!isFire) { // Smoke specific
                    effBuoyancy = 0.0001;
                    effDissipation = 0.015; // Default for non-persisted text
                }

                if (isCurrentlyPersistingText) {
                    // Overrides for "smoky" persisted text
                    effSpread = isFire ? 0.6 : 0.5; // Larger particles for smoky look
                    effTurbulence = 0.2;          // More inherent 'waviness' for jitter calc base
                    if (!isFire) {
                        effDissipation = 0.01;    // Smoke dissipates slowly
                        effBuoyancy = 0.00001; 
                    }
                    effSpeed = 0.0001; 
                }
              ...
              const sOpacity = isNaN(effOpacity)?(isFire?0.7:0.5):effOpacity;
              const sSpread = isNaN(effSpread)?1:effSpread;
              const bLifespanBase = isFire?BASE_FIRE_LIFESPAN:BASE_SMOKE_LIFESPAN;
              const sSpeed = isNaN(effSpeed)?(isFire?0.03:0.015):effSpeed;
              const sTurbulence = isNaN(effTurbulence)?1:effTurbulence;
              const sDissipation = isNaN(effDissipation ?? (isFire ? 0 : 0.15)) ? (isFire ? 0: 0.15) : (effDissipation ?? (isFire ? 0 : 0.15));
              const sBuoyancy = isNaN(effBuoyancy ?? (isFire ? 0 : 0.005)) ? (isFire ? 0 : 0.005) : (effBuoyancy ?? (isFire ? 0 : 0.005));
              ...
              for (let i=0; i<particleCount; ++i) {
                  const i3=i*3;
                  const particleRandFactor1 = randFactorsArr[i3];
                  const particleRandFactor2 = randFactorsArr[i3+1];
                  const particleRandFactor3 = randFactorsArr[i3+2];

                  const bLifespan = bLifespanBase * (0.7 + particleRandFactor1 * 0.3);


                  if (isCurrentlyPersistingText && targetPtArr && textPointsRef.current.length > 0) {
                      lifeArr[i] -= (safeDelta * (15 + sDissipation * 40));
                      lifeUpd = true;

                      if (lifeArr[i] <= 0 || isNaN(lifeArr[i]) || isNaN(posArr[i3+1])) { // check posArr for NaN too
                        const point = textPointsRef.current[i % textPointsRef.current.length];
                        ...
                        lifeArr[i] = bLifespan; // Reset life
                        alphaArr[i] = sOpacity * 0.1; // Start almost transparent for fade-in
                        alphaUpd = true;

                        const targetSizePersist = sSpread * (isFire ? 0.8 : 0.7);
                        ...
                        // Size can remain constant or also vary with lifeRatio if desired
                        const targetSizePersist = sSpread * (isFire ? 0.8 : 0.7);
                  ...
                  } else { // Normal simulation or non-persisted text
                      ...
                      if(cLife <= 0 || posArr[i3+1] > respawnHeight || isNaN(cLife) || isNaN(posArr[i3+1])){
                         ...
                         if (isTextShapingMode && !isCurrentlyPersistingText) { // non-persisted text
                            rAlpha = sOpacity; // Use the text mode opacity
                            rSize = sSpread * (isFire ? 0.75 : 0.7);
                         }
                         ...
                         if (currentSourceType === "Text" && textPointsRef.current.length > 0 && targetPtArr) {
                         ...
                         const lifeRatio = Math.max(0, Math.min(1, cLife / Math.max(0.01, bLifespan)));
                        ...
                        if (isTextShapingMode && !isCurrentlyPersistingText) { // non-persisted text
                            currentAlphaVal = sOpacity;
                            currentSizeVal = sSpread * (isFire ? 0.75 : 0.7);

                            const fadeOutStartText = 0.4;
                            const fadeDurationText = 1.0 - fadeOutStartText;
                            if (lifeRatio < fadeOutStartText && fadeDurationText > 0) {
                                ...
                                currentAlphaVal *= Math.max(0, 1.0 - (lifeRatio - fadeOutStartText) / fadeDurationText);
                            } else if (fadeDurationText <= 0) {
                               // no fade
                            } else {
                                 currentAlphaVal = 0;
                            }
                        } else { // Normal simulation (not text shaping OR text shaping is persisting, handled above)
              ...
              if(targetUpd)attrs.targetTextPoint.needsUpdate = true;
          };
       ...
       try {
           ...
           setLoadError("Render error occurred. Please refresh or check console.");
       }
     };
     animate();
